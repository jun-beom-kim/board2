...
public class CommentService {
    @Autowired
    private CommentRepository commentRepository; // 댓글 관리 레포지토리

    @Autowired
    private ArticleRepository articleRepository; // 게시글 관리 레포지토리

    public List<CommentDto> comments(Long articleId) {
//        // articleId 로 게시글에 달린 댓글 조회
...
//        return dtos;
    ...
    public CommentDto create(Long articleId, CommentDto dto) {
        // 1. 게시글이 존재하는지 확인
        // null 이 될 수 있는 obj(객체) = Optional 객체 - 값이 있으면 반환, 없으면 예외
    ...
    public CommentDto update(Long id, CommentDto dto) {
        ...
        // 변경된 데이터를 entity -> dto 변환 후 반환

        return CommentDto.createCommentDto(updateData);
    }
    @Transactional
    public CommentDto delete(Long id) {
        Comment dbData = commentRepository.findById(id).orElseThrow(() -> new IllegalArgumentException("댓글 삭제에 실패하였습니다." + "요청하신 댓글이 존재하지 않습니다."));
